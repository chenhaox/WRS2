...
| 脚本 | 演示内容 |
| --- | --- |
| `examples/virtual_d405_rgbd.py` | 在一个 Matplotlib 窗口中并排显示同一帧 RGB、深度图；色条单位 m，无效深度显示为黑色 |
...
图像示例需要可选的 `matplotlib`；未安装时运行
`.venv\Scripts\python.exe -m pip install matplotlib`。另外两个示例只用项目已有依赖。
逐个运行 Viewer 示例，在终端按 Ctrl+C 结束当前脚本。鼠标拖动可从不同方向观察点云。
