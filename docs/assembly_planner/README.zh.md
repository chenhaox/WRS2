...
状态：**M1（00—03）已实现并完成追加审查；M2 的 04、05 已实现，06—07 待实施；M3 待实施**。2026-09-11。
...
先看 [M1 使用方法、例子与交接](m1.zh.md) 和 [ContactModel 多后端与 SDF 设计](contact-backends.zh.md)。现已提供 mesh / SDF 两个算法后端、Open3D 网格字段与原生 GridSDF 输入，并新增接触图、局部方向、静力平衡与有限支撑需求；尚不包含有限路径、装配序列或机器人执行求解。
...
每个新对话先阅读本文件、[接口约定](contracts.zh.md)、[M1 交接](m1.zh.md) 和 [方向与 M2.05 交接](directions-and-stability.zh.md)，再执行对应任务文档。00—05 已落地，06—10 的未来 API 尚未实现。本轮没有创建额外对话。
...
- **M2：几何与静力装配规划（04—07），04、05 已完成**。已提供接触图、局部方向、静力证据和有限支撑需求；06—07 将输出零件路径和序列，执行状态仍需单独验证。
...
各 task 已附完整启动提示词。下一对话从 [06：有限零件运动](tasks/06-part-motion.zh.md) 开始，并读取 [方向与 M2.05 交接](directions-and-stability.zh.md)。00—05 的提示词保留为设计记录，不需要重新从零实现。
