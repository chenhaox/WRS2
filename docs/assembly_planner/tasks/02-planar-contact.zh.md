...
状态：**M1 中已完成，2026-09-10，已与 03 整合**。前置：01。以下为原设计记录，实际接口见 [M1 交接](../m1.zh.md)。
...
实现投影三角交集、真实单元覆盖、孔与多组件、仿射 gap 分带、独立线/点、面积质心积分权重及 provenance；候选筛选使用 AABB/KD-tree，仅作空间加速。没有调用 collision manifold。

`test_planar_contact.py` 8 项通过；完整箱面 0.01 m²、部分接触 0.0075 m²、方环 0.0084 m²；正间隙默认 near，显式 idealize 才能激活；包含交换/整体变换、重三角化、倾斜/线/点。曲面测试另覆盖平面预算耗尽和几何误差跨零。

dispatcher 同时返回 03 的独立 overlap 诊断。平面报告 cell/loop 是真实交集，HTML/WRS 显示直接消费结果。CLI、容差/投影面积含义、输出示例见 M1 交接。
