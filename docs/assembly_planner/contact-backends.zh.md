...
默认 Open3D 字段在归一化局部坐标下计算，降低大世界坐标对小间隙的舍入影响。它直接调用 `compute_signed_distance` / `compute_closest_points`，没有生成体素或点云。Open3D 查询为 float32，代码中的距离 guard 是工程估计，不能标为形式化精度证明；法线同样是采样验证。因此即使轴壁完整可见，结果质量仍为 estimated。
