...
Implementation files are grouped by responsibility; see [package layout](../../wrs/assembly/README.md). Previous public module paths remain compatibility aliases, except the removed legacy adapter.
...
## `wrs.assembly.mechanics._batched_lp`
_Small related LPs: common feasible basis, batched simplex, certificates._

- `starting_basis(matrix, rhs, tolerance=1e-09)` — Find one feasible basis of the shared alpha=0 system, or return None.
- `simplex(tableau, basis, *, xp, max_iterations=256, pivot_tolerance=1e-11, optimality_tolerance=1e-10)` — All LPs pivot together; inactive/converged rows remain unchanged.
- **class `BatchedLP`** — Related max c*x LPs, [common, varying columns] x = rhs, x>=0.
  - methods: `solve`

## `wrs.assembly.motion.constraints`
...
## `wrs.assembly.motion.directions`
...
## `wrs.assembly.robotics.execution`
...
## `wrs.assembly.mechanics.force_points`
...
## `wrs.assembly.robotics.graspability`
...
## `wrs.assembly.motion.part_motion`
...
## `wrs.assembly.geometry.primitives`
...
## `wrs.assembly.planning.quality_search`
...
## `wrs.assembly.planning.sequence`
...
## `wrs.assembly.mechanics.equilibrium`
...
## `wrs.assembly.mechanics.stability_sweep`
...
- **class `DirectionalStabilityAnalyzer`** — Compile contact sites/matrices once; reuse for many direction batches.
  - methods: `analyze`
...
_Explicit rigid rotation conventions; independent of legacy file formats._

- `rotation_xyz(rotation_rad)` — Extrinsic xyz radians; Rz @ Ry @ Rx.

Old-data conversion is now a repository tool: `tools/assembly/legacy_data.py`.
Runtime consumers use `load_assembly` with native `wrs.assembly/1` manifests.
...
- `closest_points(points, triangles)`
- `segment_pairs(p, q, r, s)`
- `triangle_pairs(a, b, tol=1e-10)` — N pairs -> two N×3 nearest witnesses; closed surface distance only.
