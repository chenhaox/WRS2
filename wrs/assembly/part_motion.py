...
from .model import AssemblyState, checked_tf, readonly, freeze, digest
...
class MotionConfig:
    clearance_m: float = .0002
    outside_margin_m: float = .02
    ...
    time_limit_s: float = 20.
    search_se3: bool = True
    detour_scale: float = .3
    rotation_angle_rad: float = np.pi/4

    def __post_init__(self):
        for k in ('clearance_m','outside_margin_m','min_step_m','max_translation_step_m',
                  'max_rotation_step_rad','numerical_tol_m','time_limit_s','detour_scale','rotation_angle_rad'):
            v = getattr(self,k)
        ...
        for k in ('max_nodes','max_candidates','max_triangle_tests'):
            if type(getattr(self,k)) is not int or getattr(self,k)<1: raise ValueError(k+' must be a positive integer')
...
class RemovalAction:
    ...
    def __post_init__(self):
        ids = (self.moving_part_ids,) if isinstance(self.moving_part_ids,str) else tuple(self.moving_part_ids)
        if not ids or len(set(ids))!=len(ids): raise ValueError('Unique moving part IDs required')
...
def _binding(assembly,state,moving):
    ...
    return digest((state.world_revision,tuple((k,parts[k].geometry.geometry_id,
        None if k==moving else state.poses[k]) for k in sorted(state.poses))))
...
class ContactPolicy:
    ...
    def __post_init__(self):
        object.__setattr__(self,'reference_rotation',readonly(self.reference_rotation,shape=(3,3)))
        object.__setattr__(self,'planes',freeze(self.planes))

    @classmethod
    def from_graph(cls,assembly,state,moving_part_id,graph,*,backend=None,tolerance_m=1e-9):
        graph.assert_matches(assembly,state)
        backend = backend or MeshProximity()
        parts = {p.part_id:p for p in assembly.parts}
        planes = []
        for edge in graph.edges:
            if moving_part_id not in edge.part_ids: continue
            for patch in edge.patches:
                if (patch.classification!='active' or patch.quality not in ('analytic','bounded')
                        or patch.provenance.get('idealize_contact',False) or not len(patch.normals_a_world)):
                    continue
                ...
                if np.max(va@n-h)>tolerance_m or np.min(vb@n-h)<-tolerance_m: continue
                surface_ids = (patch.surface_a_id,patch.surface_b_id) if moving_b else (patch.surface_b_id,patch.surface_a_id)
                valid = True
                for part,tf,sid in ((pa,state.poses[other],surface_ids[0]),(pb,state.poses[moving_part_id],surface_ids[1])):
                    prep,surfaces = backend.prepare(part.geometry)
                    found = next((s for s in surfaces if s.patch_id==sid and s.kind=='plane'),None)
                    ...
                    if not len(supporting) or not set(supporting).issubset(set(found.face_ids)):
                        valid=False; break
                if valid:
                    planes.append(dict(obstacle_id=other,normal=n,offset_m=h,
                                       obstacle_surface_id=surface_ids[0],moving_surface_id=surface_ids[1]))
        return cls(moving_part_id,_binding(assembly,state,moving_part_id),state.poses[moving_part_id][:3,:3],tuple(planes))

    def assert_matches(self,assembly,state,moving):
...
class PathValidation:
    ...
    def __post_init__(self):
        for k in ('diagnostics','witness'): object.__setattr__(self,k,freeze(getattr(self,k)))
...
def interpolate_pose(a,b,t):
    ...
    out[:3,:3] = Slerp([0.,1.],Rotation.from_matrix(np.stack((a[:3,:3],b[:3,:3]))))([t]).as_matrix()[0]
    return out


def sample_path(poses,config=None):
    cfg = config or MotionConfig()
    out = [checked_tf(poses[0])]
    for a,b in zip(poses,poses[1:]):
        angle = Rotation.from_matrix(b[:3,:3]@a[:3,:3].T).magnitude()
        count = max(1,int(np.ceil(np.linalg.norm(b[:3,3]-a[:3,3])/cfg.max_translation_step_m)),
...
def validate_object_path(assembly,state,moving_part_id,poses,*,policy=None,config=None,backend=None):
    ...
    """
    cfg = config or MotionConfig(); backend = backend or MeshProximity(numerical_tol_m=cfg.numerical_tol_m)
    start = perf_counter(); budget = QueryBudget(cfg.max_triangle_tests)
    path = tuple(checked_tf(p) for p in poses)
    if not path: raise ValueError('A path needs at least one pose')
    ...
    if policy is not None: policy.assert_matches(assembly,state,moving_part_id)
    parts = {p.part_id:p for p in assembly.parts}; moving = parts[moving_part_id]
    obstacles = [(p,state.poses[p.part_id],_world(p,state.poses[p.part_id]),
                  _normals(p.geometry,state.poses[p.part_id])) for p in assembly.parts
    ...
    intervals,certs = 0,dict(swept_aabb=0,separating_plane=0,allowed_surface=0,distance_bound=0)

    def result(status,reason,witness=None):
        return PathValidation(status,reason,dict(elapsed_s=perf_counter()-start,intervals=intervals,
            triangle_tests=budget.used,certificates=certs,clearance_m=cfg.clearance_m),witness or {},
    ...
    while pending:
        ...
        lo,hi = np.minimum(va.min(0),vb.min(0))-inflate,np.maximum(va.max(0),vb.max(0))+inflate
        midpoint = None; split = False
        for obstacle,otf,ov,other_normals in obstacles:
            ...
            if aabb_distance(lo,hi,ov.min(0),ov.max(0))>cfg.clearance_m:
                certs['swept_aabb']+=1; continue
            if angle<=1e-12:
                if policy is not None and np.linalg.norm(a[:3,:3]-policy.reference_rotation,ord=2)*radius<=cfg.numerical_tol_m:
                    permits = [x for x in policy.planes if x['obstacle_id']==obstacle.part_id]
                    if any(min(np.min(va@x['normal']-x['offset_m']),np.min(vb@x['normal']-x['offset_m']))
                           >=-cfg.numerical_tol_m for x in permits):
                        certs['allowed_surface']+=1; continue
                axes = np.vstack((local_normals@a[:3,:3].T,other_normals,np.eye(3),-np.eye(3)))
                ...
                if len(axes)>256: axes=axes[np.linspace(0,len(axes)-1,256,dtype=int)]
                separated=False
                batch=max(1,min(32,262144//max(len(va),len(ov))))
                for first in range(0,len(axes),batch):
                    directions=axes[first:first+batch].T
                    gap=np.minimum(np.min(va@directions,axis=0),np.min(vb@directions,axis=0))-np.max(ov@directions,axis=0)
                    if np.max(gap)>cfg.clearance_m:
                        separated=True; break
                if separated:
                    certs['separating_plane']+=1; continue
            if midpoint is None: midpoint=interpolate_pose(a,b,.5)
            overlap = backend.classify_overlap(moving,midpoint,obstacle,otf,budget=budget)
            if overlap.status=='penetrating':
                return result('blocked',overlap.reason,dict(witness,pose=midpoint,point=overlap.witness_world_m))
            if overlap.status=='unknown': return result('unknown',overlap.reason,witness)
            distance = backend.pair_distance(moving,midpoint,obstacle,otf,budget=budget)
            if distance.status!='complete': return result('unknown','distance_budget_exhausted',witness)
            if distance.lower_bound_m-movement>cfg.clearance_m:
                certs['distance_bound']+=1; continue
            if movement<=cfg.min_step_m:
                return result('unknown','unresolved_clearance_or_unpermitted_contact',dict(witness,
                    midpoint_gap_m=distance.lower_bound_m,point_motion_bound_m=movement))
            split=True; break
        if split: pending.extend(((a,midpoint),(midpoint,b)))
...
def _outside_pose(moving,tf,obstacles,direction,margin):
    ...
    distance = margin if not len(obstacles) else max(margin,float(np.max(obstacles@d)-np.min(vertices@d)+margin))
    out = tf.copy(); out[:3,3]+=d*distance
    return out


def plan_removal(assembly,state,action,*,graph=None,backend=None,config=None,directions=None):
    ...
    if len(action.moving_part_ids)!=1 or action.primitive!='rigid':
        return RemovalResult('unsupported',action.moving_part_ids[0],(),None,None,key,{'reason':'single_rigid_part_only'})
    moving_id = action.moving_part_ids[0]
    _binding(assembly,state,moving_id)
    parts = {p.part_id:p for p in assembly.parts}; part = parts[moving_id]
    if part.fixed: return RemovalResult('unsupported',moving_id,(),None,None,key,{'reason':'fixed_part'})
    graph = graph or build_contact_graph(assembly,state,analyze_contacts(assembly,state,backend=backend))
    graph.assert_matches(assembly,state)
    policy = ContactPolicy.from_graph(assembly,state,moving_id,graph,backend=backend,tolerance_m=cfg.numerical_tol_m)
    started=perf_counter(); attempts=[]
    seeds=[]
    if directions is None:
        local=assembly_directions(graph,(moving_id,))
        if local.best_direction is not None: seeds.append(local.best_direction)
    ...
    for d in seeds:
        ...
        if not any(np.allclose(d,x,rtol=0,atol=1e-12) for x in unique): unique.append(d)
    others=[_world(p,state.poses[p.part_id]) for p in assembly.parts if p.part_id in state.poses and p.part_id!=moving_id]
    ...
    if cfg.search_se3 and directions is None:
        size=max(.01,float(np.linalg.norm(np.ptp(np.vstack([_world(part,tf),ov]),axis=0))))*cfg.detour_scale
        for axis in np.vstack((np.eye(3),-np.eye(3))):
            waypoint=tf.copy(); waypoint[:3,3]+=axis*size
            for d in unique:
                candidates.append((tf,waypoint,_outside_pose(part,waypoint,ov,d,cfg.outside_margin_m)))
        # Bounded orientation candidates use genuine quaternion interpolation.
        rotations=[]
        for axis in np.vstack((np.eye(3),-np.eye(3))):
            waypoint=tf.copy(); waypoint[:3,:3]=Rotation.from_rotvec(axis*cfg.rotation_angle_rad).as_matrix()@tf[:3,:3]
            rotations.append((tf,waypoint,_outside_pose(part,waypoint,ov,unique[0],cfg.outside_margin_m)))
        candidates=candidates[:len(unique)]+rotations+candidates[len(unique):]
    for path in candidates[:cfg.max_candidates]:
        if perf_counter()-started>cfg.time_limit_s: break
        remaining=max(1e-9,cfg.time_limit_s-(perf_counter()-started))
        validation=validate_object_path(assembly,state,moving_id,path,policy=policy,
                                        config=replace(cfg,time_limit_s=remaining),backend=backend)
        attempts.append(dict(status=validation.status,reason=validation.reason,diagnostics=validation.diagnostics))
        if validation.status=='valid':
            return RemovalResult('success',moving_id,sample_path(path,cfg),policy,validation,key,
                                 dict(attempts=attempts,elapsed_s=perf_counter()-started,outside='directional_support_plane'))
    return RemovalResult('unknown',moving_id,(),policy,None,key,
                         dict(reason='bounded_search_exhausted',attempts=attempts,elapsed_s=perf_counter()-started))
