...
from ..model import ContactConfig, ContactAnalysis, Part, digest, checked_tf
...
class ContactBackend(Protocol):
    ...
    def analyze_pair(self, a: ContactModel, tf_a, b: ContactModel, tf_b,
                     *, config: ContactConfig) -> ContactAnalysis: ...
...
class MeshContactBackend:
    ...
    def __init__(self, *, proximity=None, geometry_config=None):
    ...
    def analyze_pair(self, a, tf_a, b, tf_b, *, config):
        ...
        return analyze_pair(a.as_part(), tf_a, b.as_part(), tf_b,
                            config=config, backend=self.proximity)
...
class ContactAnalyzer:
    ...
    """
    def __init__(self, backend='mesh', *, config=None):
        self.backend = resolve_backend(backend)
        self.config = config or ContactConfig()

    def analyze_pair(self, a, b, *, tf_a=None, tf_b=None):
        ...
        key = digest((a.name, a.geometry_key, ta, b.name, b.geometry_key, tb,
                      self.config, self.backend.cache_key))
        diagnostics = tuple({**d, 'contact_backend': self.backend.name,
                             'backend_key': self.backend.cache_key} for d in result.pair_diagnostics)
        return replace(result, state_digest=key, pair_diagnostics=diagnostics)

    def analyze(self, models, *, poses=None):
        ...
        results = [self.analyze_pair(by_id[a], by_id[b], tf_a=poses[a], tf_b=poses[b])
                   for a, b in combinations(sorted(poses), 2)]
        return ContactAnalysis(tuple(p for r in results for p in r.patches),
                               tuple(d for r in results for d in r.pair_diagnostics),
                               digest(([(m.name, m.geometry_key) for m in models], poses,
                                       self.config, self.backend.cache_key)),
                               statistics={'pair_count': len(results),
                                           'pairs': [r.statistics for r in results]})
