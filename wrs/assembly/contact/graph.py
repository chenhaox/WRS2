...
from ..model import digest, freeze


def state_geometry_binding(assembly, state):
    ...
    return {'schema': 'assembly_geometry_state/1', 'world_revision': state.world_revision,
            'instances': {k: {'geometry_id': parts[k].geometry.geometry_id, 'tf': state.poses[k]}
...
class ContactGraph:
    ...
    def assert_matches(self, assembly, state):
        ...
        mates = tuple(m for m in assembly.mating_relations if m.part_a in fixed and m.part_b in fixed)
...
def build_contact_graph(assembly, state, analysis):
    ...
    if digest(analysis.input_binding) != digest(binding):
        raise ValueError('Analysis is stale or unbound; recompute analyze_contacts(assembly, state)')
    parts = {p.part_id: p for p in assembly.parts}
    ...
    for ids in combinations(sorted(nodes), 2):
        ...
        for d in diagnostics[ids]:
            ...
            if (d.get('active_area', {}).get('status') == 'not_solved'
                    or (status == 'touching' and not any(p.classification == 'active' for p in patches[ids]))):
        ...
        edges.append(ContactEdge(ids, tuple(patches[ids]), tuple(mates[ids]),
                                 tuple(diagnostics[ids]), tuple(sorted(issues))))
    key = digest(('contact_graph/1', binding, analysis.state_digest,
                  {k: n['fixed'] for k, n in nodes.items()}, tuple(m for e in edges for m in e.mating_relations)))
