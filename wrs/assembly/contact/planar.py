...
from ..geometry.planar import (polygon_measure, convex_intersection, clip_halfplane,
                               clean_polygon, cell_regions)
...
def analyze_planar_pair(part_a, tf_a, prep_a, surface_a,
                        part_b, tf_b, prep_b, surface_b, *, config, budget=None):
    ...
    error = (prep_a.mesh.geometry_error_m + prep_b.mesh.geometry_error_m
             + sa.residual_m + sb.residual_m + cfg.pose_error_m) / abs(denom)
    ...
    for i, tri in enumerate(ta):
        ...
        for j in sorted(tree.query_ball_point(center, radius + radii_b.max() + eps)):
            ...
            if parallel:
                ...
                cuts = sorted(set([float(gs.min()), float(gs.max()), -error-eps,
                                   error+eps, -cfg.penetration_tol_m-error, 0.0,
                                   -cfg.contact_tol_m+error, cfg.contact_tol_m-error]))
    ...
    for label, records in sorted(labeled.items()):
        ...
        provenance = {'backend': 'planar/1', 'area_chart': 'part_a_plane',
                      'geometry_a': part_a.geometry.geometry_id, 'geometry_b': part_b.geometry.geometry_id,
                      'source_face_ids_a': sorted({k for i in sa.face_ids for k in prep_a.source_face_ids[i]}),
                      'source_face_ids_b': sorted({k for i in sb.face_ids for k in prep_b.source_face_ids[i]}),
                      'cell_prepared_face_pairs': face_pairs, 'error_bound_m': error,
                      'boundary_valid': boundary_ok, 'idealize_contact': cfg.idealize_contact}
        ...
        def make(cells, loops, dimension, weights, length=0.0):
            centroids = np.asarray([polygon_measure(c)[1] if dimension == 2 else c.mean(0) for c in cells])
            witnesses_a, witnesses_b = world_a(centroids), world_b(centroids)
            all_gaps = np.concatenate([gap(c) for c in cells])
            regions = tuple(Region(tuple(world_a(area_cells[i]) for i in ids),
                                   tuple(world_a(loop) for loop in ring)) for ids, ring in loops) if dimension == 2 else (
                                       Region(tuple(world_a(c) for c in cells)),)
            return ContactPatch(part_a.part_id, part_b.part_id, sa.patch_id, sb.patch_id,
                                dimension, label, 'bounded' if boundary_ok and not exhausted else 'unresolved',
                                witnesses_a, witnesses_b, np.tile(na, (len(centroids), 1)),
                                np.tile(nb, (len(centroids), 1)), regions,
                                (float(all_gaps.min()-error), float(all_gaps.max()+error)),
                                float(np.sum(weights)) if dimension == 2 else 0.0, length,
                                np.asarray(weights), 'contact' if label == 'active' else 'projected_region',
        ...
        if segments:
            result.append(make(segments, (), 1, [], sum(np.linalg.norm(s[1]-s[0]) for s in segments)))
        unique_points = []
        for p in points:
            if covered(p) or any(np.linalg.norm(p-q) <= eps for q in unique_points):
                continue
            if any(np.linalg.norm(p-(s[0]+np.clip(np.dot(p-s[0], s[1]-s[0])/np.dot(s[1]-s[0], s[1]-s[0]), 0, 1)*(s[1]-s[0]))) <= eps for s in segments):
                continue
            unique_points.append(p)
        if unique_points:
            result.append(make([np.asarray([p]) for p in unique_points], (), 0, []))
    return tuple(result), {'candidate_triangle_pairs': candidates, 'unresolved_area_m2': unresolved,
