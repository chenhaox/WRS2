...
class ContactModel:
    ...
    def __post_init__(self):
        ...
        if any(not isinstance(k, str) or not isinstance(getattr(v, 'cache_key', None), str)
    ...
    def geometry_key(self):
        return digest((self.geometry.geometry_id,
                       {k: v.cache_key for k, v in self.representations.items()}))
    ...
    @classmethod
    def from_file(cls, path, *, name, length_unit, **kwargs):
    ...
    @classmethod
    def from_arrays(cls, vertices, faces, *, name, length_unit, **kwargs):
    ...
    @classmethod
    def from_scene_object(cls, obj, *, name=None, geometry_error_m=0.0):
        ...
        mesh = MeshData(np.concatenate(vertices), np.concatenate(faces),
