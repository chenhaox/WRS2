...
def _roundoff_length(points):
    return 64*np.finfo(float).eps*max(.001,float(np.max(np.abs(points),initial=0)))
...
def _clean(patch):
    ...
    if len(p)>1:
        ...
        ids.sort()
        p,n = p[ids],n[ids]
    return ForcePoints(p,n,dict(input_points=len(points),exact_unique_points=exact_count,
        clean_points=len(p),roundoff_weld_tolerance_m=tol,method='clean_only',curved_subset=False))


def _planar_selection(p,n,dimension,tol):
    if len(p)<2 or not np.all(n==n[0]): return None
    centered = p-p[0]
    if dimension==1:
        far = int(np.argmax(np.einsum('ij,ij->i',centered,centered)))
        direction = centered[far]
        length = np.linalg.norm(direction)
        if length==0: return np.array([0]),'point'
    ...
    if dimension!=2 or np.max(np.abs(centered@n[0]))>tol: return None
    if len(p)<=3: return np.arange(len(p)),'small_planar'
    axis = np.eye(3)[np.argmin(np.abs(n[0]))]
    tangent = np.cross(n[0],axis); tangent /= np.linalg.norm(tangent)
...
def _curved_selection(p,n,patch,budget,spacing,normal_angle):
    count = len(p)
    if count<=budget: return np.arange(count),dict(method='small_curved',curved_subset=False)
    ...
    if boundary:
        ...
        priority[ids] = True
    extrema = np.r_[np.argmin(p,axis=0),np.argmax(p,axis=0),np.argmin(n,axis=0),np.argmax(n,axis=0)]
    priority[extrema] = True
    best = np.full(count,np.inf)
    selected = np.zeros(count,dtype=bool)
    indices = []
    for _ in range(min(count,budget)):
        candidates = priority & ~selected & (best>1.)
        score = np.where(candidates,best,-1.) if np.any(candidates) else best
        index = int(np.argmax(score))
        if len(indices)>=4 and best[index]<=1.: break
        indices.append(index); selected[index] = True
        delta = features-features[index]
        np.minimum(best,np.einsum('ij,ij->i',delta,delta),out=best)
        best[selected] = -1.
    return np.sort(indices),dict(method='curved_fps',curved_subset=len(indices)<count,
        boundary_priority_points=int(np.count_nonzero(priority)),
        max_normalized_cover_distance=float(np.sqrt(max(0.,np.max(best)))),
        coverage_target_met=bool(np.max(best)<=1.))


def prepare_force_points(patch,*,reduce=True,curved_budget=64,spacing=.005,
                         normal_angle=np.pi/18,full_curves=False):
    key = (bool(reduce),curved_budget,spacing,normal_angle,bool(full_curves))
    with _LOCK:
        entry = _CACHE.get(patch)
        if entry is not None and key in entry: return entry[key],True
        clean = entry.get('clean') if entry is not None else None
    if clean is None: clean = _clean(patch)
    ...
    with _LOCK:
        if patch not in _CACHE:
            if len(_CACHE)>=_MAX_PATCHES: del _CACHE[next(iter(_CACHE))]
            _CACHE[patch] = {'clean':clean}
        entry = _CACHE[patch]
        if len(entry)>_MAX_POLICIES: entry.clear(); entry['clean'] = clean
        entry[key] = result
    return result,False
