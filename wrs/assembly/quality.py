...
def score_assemblability(directions, *, profile='asp_old'):
    ...
    if directions.status == 'blocked' and k == 0:
        ...
        return AssemblabilityScore('unknown', '?', 'unresolved', None, profile)
    if profile == 'wan2016':
        if case in ('free', 'a', 'c', 'f'): value = 'infinity'
        elif case == 'b': value = 10
...
class StepQuality:
    ...
    def __post_init__(self):
        if (not math.isfinite(self.stability) or self.stability < 0 or
                not math.isfinite(self.assemblability) or self.assemblability < 0 or
                type(self.graspability) is not int or self.graspability < 0):
            raise ValueError('Use finite nonnegative qualities; unresolved results are not zero')


def sequence_quality(steps, *, support_penalty=100.):
    ...
    steps = tuple(steps)
    if not steps: raise ValueError('A scored prefix must contain a step')
...
def quality_upper_bound(steps, *, support_penalty=100., remaining_steps):
