...
from examples.assembly.m2_contact_graph_demo import fixtures
from wrs.assembly import ContactConfig, analyze_contacts, build_contact_graph
from wrs.assembly.constraints import candidate_motions
...
def main():
    ...
    parser.add_argument('--solver-path', type=Path,
                        default=ROOT/'examples/assembly/output/direction-methods/deps')
    parser.add_argument('--out', type=Path,
                        default=ROOT/'examples/assembly/output/direction-methods/summary.json')
    args = parser.parse_args()
    if args.samples < 1 or args.repeat < 3:
        parser.error('Use positive samples and at least 3 repeats')
    if args.solver_path.is_dir():
        sys.path.insert(0, str(args.solver_path))
    try:
        import clarabel
    except ImportError:
        clarabel = None
    directions = fibonacci_sphere(args.samples)
    np.testing.assert_allclose(np.linalg.norm(directions, axis=1), 1, rtol=0, atol=1e-14)
    np.testing.assert_array_equal(directions, fibonacci_sphere(args.samples))
    cases, production = {}, {}
    ...
    for key, description, assembly, backend, cfg in fixtures()[:4]:
        ...
        production[name] = dict(timing=timing(lambda: candidate_motions(graph, ['part'], config=cfg), args.repeat),
    ...
    for name, normals in cases.items():
        ...
        row = dict(case=name, normalized_rows=normals.tolist(), rows=len(normals),
                   sample=sampled, six_lp_nonzero=bool(len(found)), six_lp=timing(lambda: six_lp(normals), args.repeat))
        row['sample_cached'] = timing(lambda: sample_result(normals, directions), args.repeat)
        row['sample_generate_and_filter'] = timing(
            lambda: sample_result(normals, fibonacci_sphere(args.samples)), args.repeat)
        if clarabel:
            for formulation in ('socp', 'qp'):
                ...
                row[formulation] = dict(result=result, timing=timing(
                    lambda: solve_margin(normals, clarabel, formulation), args.repeat))
        ...
        for threads in ('default', 1):
            ...
            with context:
                for method in ('matmul', 'einsum', 'blocked', 'progressive'):
                    ...
                    kernels.append(dict(rows=count, samples=args.samples, method=method,
                                        blas_threads=threads, timing=timing(
                                            lambda: sample_result(normals, directions, method), args.repeat)))
        if clarabel:
            ...
            large_solvers.append(dict(rows=count, sample=sampled, optimal=optimal,
                                      six_lp=timing(lambda: six_lp(normals), args.repeat),
                                      socp=timing(lambda: solve_margin(normals, clarabel), args.repeat),
                                      qp=timing(lambda: solve_margin(normals, clarabel, 'qp'), args.repeat)))
    ...
    with threadpool_limits(limits=1, user_api='blas'):
        for method in ('matmul', 'blocked', 'progressive'):
            ...
            broad.append(dict(rows=2048, samples=args.samples, feasible_count=result['count'],
                              method=method, blas_threads=1, timing=timing(
                                  lambda: sample_result(normals, directions, method), args.repeat)))
    scaling = []
    normals = cases['corner_translation']
    for count in sorted(set((650, args.samples, 65000))):
        bank = fibonacci_sphere(count)
        sampled = sample_result(normals, bank)
        scaling.append(dict(samples=count, sample=sampled,
                            generate=timing(lambda: fibonacci_sphere(count), args.repeat),
                            cached_filter=timing(lambda: sample_result(normals, bank), args.repeat)))
    report = dict(timestamp_utc=datetime.now(timezone.utc).isoformat(), python=sys.executable,
                  ...
                  blas=threadpool_info(), samples=args.samples, tolerance=TOL, repeat=args.repeat,
