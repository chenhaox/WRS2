...
def main():
    ...
    parser.add_argument('--phase-repeat', type=int, default=15)
    parser.add_argument('--out', type=Path,
                        default=ROOT/'examples/assembly/output/direction-benchmark/summary.json')
    args = parser.parse_args()
    if min(args.repeat, args.phase_repeat) < 3:
    ...
    for key, description, assembly, backend, cfg in fixtures():
        ...
        phases = []
        for _ in range(args.phase_repeat):
        ...
        row = dict(case=key, mode=cfg.mode, description=description,
                   ...
                   first_call_for_case_ms=first_ms, repeat=args.repeat,
                   warm_median_ms=float(np.median(times)), warm_p95_ms=float(np.percentile(times, 95)),
                   raw_warm_ms=times, phase_repeat=args.phase_repeat,
