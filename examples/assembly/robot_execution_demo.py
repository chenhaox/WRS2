"""RS007L + OR2FG7: two staged blocks, M2 sequence, WRS forward robot replay.

All dimensions/capacities below are example inputs, not measured robot limits.
Run --headless to plan and validate without a viewer or any hardware connection.
"""
import argparse
...
import numpy as np
from wrs.assembly import Assembly,Part,save_report
...
def main():
    ...
    parser.add_argument('--auxiliary',action='store_true',help='second RS007L holds the floating lower block')
    args=parser.parse_args()
    a,cell=make_demo(args.auxiliary)
    supports=() if not args.auxiliary else (AuxiliarySupport('aux_arm',
    ...
    output=Path(__file__).parent/'output'/'execution'; output.mkdir(parents=True,exist_ok=True)
    save_report(result,output/('robot_execution_auxiliary.json' if args.auxiliary else 'robot_execution.json'))
    ...
    base=wvw.World(cam_pos=(2,-2.4,1.9),cam_lookat_pos=(.5,0,.65),port=args.port)
