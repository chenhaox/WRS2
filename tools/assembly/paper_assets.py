"""One-time conversion of archived paper inputs to native assembly manifests.

Run from the repository root: python -m tools.assembly.paper_assets.
Only generated manifests are written; archived JSON/STL sources stay intact.
"""
...
import json
import numpy as np
from wrs.assembly import Assembly, Part, MeshData, save_assembly
...
from examples.assembly._shared.paper_cases import ASSETS, CATALOG
...
def _asset(key):
    if "Domino" in key:
        return "domino"
    return {"smallL 2": "smallL_2", "Cross": "cross", "Crossleft": "crossleft"}.get(
        key, key
    ).replace(" ", "_")
...
def _legacy(name, nominal):
    ...
    for key, value in data.items():
        ...
        translation = np.asarray(value["location"], dtype=float) * 0.001
        raw_tf = pose(translation, rotation_xyz(rotation))
        if nominal:
            ...
            if name == "domino_5" and abs(translation[0]) > 0.01:
                theta = np.deg2rad(20)
                translation[0] = np.sign(translation[0]) * (
                    0.01 + 0.01 * np.cos(theta) + 0.12 * np.sin(theta)
                )
                translation[2] = 0.01 * np.sin(theta)
        geometry = mesh(_asset(key), nominal)
        tf = pose(translation, rotation_xyz(rotation))
        original = mesh(_asset(key), False)
        before = original.vertices @ raw_tf[:3, :3].T + raw_tf[:3, 3]
        nominal_vertices = (
            np.rint((original.vertices + EDGE / 2) / EDGE) * EDGE - EDGE / 2
            if nominal and soma
            else original.vertices
        )
        after = nominal_vertices @ tf[:3, :3].T + tf[:3, 3]
        corrections.append(
            dict(
                part=key,
                max_world_vertex_change_m=float(np.linalg.norm(after - before, axis=1).max()),
            )
        )
        parts.append(Part(key, geometry, tf))
    note = (
        (
            "名义几何：保留原始位移；恢复 18.5 mm 网格、导出角度，重建等价体素表面消除 T 形接缝。"
            if soma
            else "原始 STL；恢复接近整度的导出角度。Domino 3 另按 20° 几何恢复相切位置。"
        )
        if nominal
        else "原始 STL 和 JSON 位姿；未修正旧数据的尺度及间隙。"
    )
    return parts, dict(
        source=f"asp/data/{name}",
        reproduction="nominal_legacy" if nominal else "raw_legacy",
        note=note,
        corrections=corrections,
    )
    ...
    for cube in sorted(occupied):
        for axis in range(3):
            for sign in (-1, 1):
                ...
                normal[axis] = sign
                if tuple(np.asarray(cube) + normal) in occupied:
                    continue
                ...
                quad = (
                    np.array(
                        [
                            center + (u * tangent + v * bitangent) / 2
...
def _grid_boundary(geometry):
    ...
    return voxel_mesh(ids[abs(winding) > 0.5])
...
def _random_parts(count, seed):
    ...
    for i in range(count):
        ...
        if occupied:
            neighbors = sorted(
                {
                    tuple(np.array(c) + n)
                    for c in occupied
                    for n in np.r_[np.eye(3, dtype=int), -np.eye(3, dtype=int)]
                }
                - occupied
            )
            candidates = []
            ...
            if not candidates:
                raise RuntimeError("No connected voxel placement")
        ...
        records.append(shape.tolist())
        parts.append(Part(f"block_{i + 1}", voxel_mesh(shape)))
    return parts, dict(
        source="deterministic voxel generator inspired by Fig.13",
...
def _bridge():
    ...
    for i, (key, value) in enumerate(data.items()):
        ...
        location = np.array(value["location"]) * 0.001
        if i in (0, 3):
            location[1:3] = (y if i == 0 else -y, z)
        parts.append(Part(key, box((width, width, 0.3)), pose(location, rotation_xyz(rotation))))
    return parts, dict(
        source="asp/data/bridge + reconstructed square beams",
        reproduction="reconstructed",
        section_m=[width, width],
        length_m=0.3,
...
def make_case(key, *, nominal=True):
    ...
    if name == "bridge":
        ...
        meta = dict(
            source="Fig.12 schematic reconstruction",
            reproduction="reconstructed",
        ...
        meta = dict(
            source="asp/objects/bigL.stl",
            reproduction="nominal_legacy" if nominal else "raw_legacy",
            note="单个 BL，用于检查体素表示与地面接触。",
        )
    else:
    ...
    if order and set(order) != set(ids):
        raise ValueError((key, order, ids))
    order = order or ids
    vertices = np.vstack(
        [p.geometry.vertices @ p.assembled_tf[:3, :3].T + p.assembled_tf[:3, 3] for p in parts]
    )
    lo, hi = vertices.min(0), vertices.max(0)
    ground = Part(
        "ground",
        box((hi[0] - lo[0] + 0.06, hi[1] - lo[1] + 0.06, 0.01)),
        pose(((lo[0] + hi[0]) / 2, (lo[1] + hi[1]) / 2, lo[2] - 0.005)),
        fixed=True,
    )
    meta.update(
        title=title,
        ...
        ground_rule="fixed box top at minimum assembled vertex z",
        length_unit="m",
    )
    return Assembly(tuple([ground, *parts]), provenance=meta)


def export_manifests() -> tuple[int, int]:
    """Rebuild both supported modes, retaining explicit unavailable raw cases."""
    ...
    for key in CATALOG:
        for nominal in (False, True):
            try:
                ...
                continue
            mode = "nominal" if nominal else "raw"
            save_assembly(assembly, ASSETS / "assemblies" / f"{key}.{mode}.assembly.json")
            written += 1
    return written, unavailable


if __name__ == "__main__":
    written, unavailable = export_manifests()
    print(f"Converted {written} paper assemblies; {unavailable} unavailable source modes.")
